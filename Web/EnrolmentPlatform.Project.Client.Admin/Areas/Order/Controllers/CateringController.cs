...
namespace EnrolmentPlatform.Project.Client.Admin.Areas.Order.Controllers
{
    public class CateringController : BaseController
    {
        ...
        {
            string s = JsonConvert.SerializeObject(new SaveCateringOrderDTO());
            return View();
        }
        /// <summary>
        /// 订单列表异步搜索
        ...
        {
            var msg = await WebApiHelper.PostAsync<HttpResponseMsg>("/api/OrderForCatering/GetScenicSupplierOrderList", JsonConvert.SerializeObject(param), ConfigurationManager.AppSettings["StaffId"].ToInt());
        ...
        public async Task<ActionResult> Detail(Guid id)
        {
            ...
            var msg = await WebApiHelper.GetAsync<HttpResponseMsg>("/api/OrderForCatering/ScenicGetCateringOrderById", parameters.Item1, parameters.Item2, ConfigurationManager.AppSettings["StaffId"].ToInt());
            if (msg.IsSuccess)
            {
                OrderDetailForCateringDTO entity = msg.Data.ToString().ToObject<OrderDetailForCateringDTO>();
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<JsonResult> ScenicHandleRefund(ScenicCateringOrderHandleRefundDTO dto)
        {
            dto.UpdateUserId = base.UserId;
            dto.UpdateUserName = base.UserAccount;
            var msg = await WebApiHelper.PostAsync<HttpResponseMsg>("/api/OrderForCatering/ScenicHandleRefund", JsonConvert.SerializeObject(dto), ConfigurationManager.AppSettings["StaffId"].ToInt());
            return Json(msg);
        }
        ...
        {
            var msg = await WebApiHelper.PostAsync<HttpResponseMsg>("/api/VerificationForCatering/ScenicGetCateringVerificationList", JsonConvert.SerializeObject(param), ConfigurationManager.AppSettings["StaffId"].ToInt());
            return msg.Data.ToString();
        }

        #region OrderExcel
        public async Task<ActionResult> ExportToExcel(CateringOrderVerificationSearchParamDTO param)
        {
            param.Page = 1;
