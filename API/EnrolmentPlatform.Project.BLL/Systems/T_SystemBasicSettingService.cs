...
namespace EnrolmentPlatform.Project.BLL.Systems
{
    public class T_SystemBasicSettingService : BaseService<T_SystemBasicSetting>, IT_SystemBasicSettingService, IInterceptorLogic
    {
        ...
        /// </summary>
        /// <returns></returns>
        public List<string> GetWithdrawRange()
        {
            List<string> list = new List<string>();
            int withdrawRange = (int)SystemBasicSettingEnum.WithdrawRange;
            T_SystemBasicSetting systemBasicSetting = this.LoadEntities(t => t.Key == withdrawRange).FirstOrDefault();
            if (systemBasicSetting != null)
            {
                var array = systemBasicSetting.Value.Split('-');
                list.AddRange(array);
            }
            return list;
        }
        ...
        /// <returns></returns>
        public ResultMsg SetWithdrawRange(WithdrawRangeDTO dto)
        {
            ResultMsg msg = new ResultMsg();
            int withdrawRange = (int)SystemBasicSettingEnum.WithdrawRange;
            T_SystemBasicSetting systemBasicSetting = this.LoadEntities(t => t.Key == withdrawRange).FirstOrDefault();
            if (systemBasicSetting == null)
            {
                systemBasicSetting = new T_SystemBasicSetting()
                {
                    Id = Guid.NewGuid(),
                    Key = withdrawRange,
                    Value = string.Format("{0}-{1}", dto.StartRange, dto.EndRange),
                    CreatorAccount = dto.UpdateUserName,
                    CreatorUserId = dto.UpdateUserId
                };
            ...
            {
                systemBasicSetting.Value = string.Format("{0}-{1}", dto.StartRange, dto.EndRange);
                systemBasicSetting.LastModifyUserId = dto.UpdateUserId;
            }
            #region  tran
            DbConnection conn = ((IObjectContextAdapter)_dbContextFactory.GetCurrentThreadInstance()).ObjectContext.Connection;
            conn.Open();
            using (var tran = conn.BeginTransaction())
            {
                try
                {
                    this.PhysicsDeleteBy(t => t.Key == withdrawRange);
                    msg.IsSuccess = this.AddEntity(systemBasicSetting) > 0;
                    if (msg.IsSuccess)
                    {
                        tran.Commit();
                    }
        ...
        /// </summary>
        /// <returns></returns>
        public ResultMsg GetH5TitleSet()
        {
            ResultMsg msg = new ResultMsg();
            int webSiteTitleForH5 = (int)SystemBasicSettingEnum.WebSiteTitleForH5;
            T_SystemBasicSetting webSiteTitleForH5_entity = this.LoadEntities(t => t.Key == webSiteTitleForH5).FirstOrDefault();
            if (webSiteTitleForH5_entity != null)
            {
                msg.Data = webSiteTitleForH5_entity.Value;
            }
            return msg;
        }
        /// <summary>
        ...
        /// <param name="dto"></param>
        /// <returns></returns>
        public ResultMsg H5TitleSet(H5TitleSetDTO dto)
        {
            ResultMsg msg = new ResultMsg();
            int webSiteTitleForH5 = (int)SystemBasicSettingEnum.WebSiteTitleForH5;
            T_SystemBasicSetting webSiteTitleForH5_entity = this.LoadEntities(t => t.Key == webSiteTitleForH5).FirstOrDefault();
            if (webSiteTitleForH5_entity == null)
            {
                webSiteTitleForH5_entity = new T_SystemBasicSetting()
                {
                    Id = Guid.NewGuid(),
                    Key = webSiteTitleForH5,
                    Value = dto.H5Title,
                    CreatorAccount = dto.UpdateUserName,
                    CreatorUserId = dto.UpdateUserId
                };
            ...
            {
                webSiteTitleForH5_entity.Value = dto.H5Title;
                webSiteTitleForH5_entity.LastModifyUserId = dto.UpdateUserId;
            }
            #region  tran
            DbConnection conn = ((IObjectContextAdapter)_dbContextFactory.GetCurrentThreadInstance()).ObjectContext.Connection;
            conn.Open();
            using (var tran = conn.BeginTransaction())
            {
                try
                {
                    this.PhysicsDeleteBy(t => t.Key== webSiteTitleForH5);
                    msg.IsSuccess = this.AddEntity(webSiteTitleForH5_entity) > 0;
                    if (msg.IsSuccess)
                    {
                        tran.Commit();
                    }
                    ...
                    RedisHelper.Remove("H5TitleSet");//清楚缓存
                }
                catch (Exception ex)
                {
                    tran.Rollback();
        ...
        /// </summary>
        /// <returns></returns>
        public ResultMsg GetUserProtocolSet()
        {
            ResultMsg msg = new ResultMsg();
            int userProtocol = (int)SystemBasicSettingEnum.UserProtocol;
            T_SystemBasicSetting userProtocol_entity = this.LoadEntities(t => t.Key == userProtocol).FirstOrDefault();
            if (userProtocol_entity != null)
            {
                msg.Data = userProtocol_entity.Value;
            }
            return msg;
        }
        ...
        /// <param name="dto"></param>
        /// <returns></returns>
        public ResultMsg UserProtocolSet(UserProtocolSetDTO dto)
        {
            ResultMsg msg = new ResultMsg();
            int userProtocol = (int)SystemBasicSettingEnum.UserProtocol;
            T_SystemBasicSetting userProtocol_entity = this.LoadEntities(t => t.Key == userProtocol).FirstOrDefault();
            if (userProtocol_entity == null)
            {
                userProtocol_entity = new T_SystemBasicSetting()
                {
                    Id = Guid.NewGuid(),
                    Key = userProtocol,
                    Value = dto.Content,
            ...
            {
                userProtocol_entity.Value = dto.Content;
                userProtocol_entity.LastModifyUserId = dto.UpdateUserId;
            }
            #region  tran
            DbConnection conn = ((IObjectContextAdapter)_dbContextFactory.GetCurrentThreadInstance()).ObjectContext.Connection;
            conn.Open();
            using (var tran = conn.BeginTransaction())
            {
                try
                {
                    this.PhysicsDeleteBy(t => t.Key == userProtocol);
                    msg.IsSuccess = this.AddEntity(userProtocol_entity) > 0;
                    if (msg.IsSuccess)
                    {
                        tran.Commit();
                    }
                    ...
                    //RedisHelper.Remove("GetUserProtocolSet");//清楚缓存
                }
                catch (Exception ex)
                {
                    tran.Rollback();
