    ...
    public class T_OrderRepository : BaseRepository<T_Order>, IT_OrderRepository
    {
        ...
        public string RejectUpload(List<OrderRejectUploadDto> list, Guid userId)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (order.Status >= (int)OrderStatusEnum.Join)
                    continue;

                //指标处理
                var stock = dbContext.T_StockSetting.FirstOrDefault(a => a.SchoolId == order.SchoolId && a.LevelId == order.LevelId
                  && a.MajorId == order.MajorId && a.BatchId == order.BatchId);
                ...
                //修改订单状态
                order.Status = (int)OrderStatusEnum.Reject;
                order.RejectReason = dto.RejectReason;
                order.ToLearningCenterId = null;
                order.LastModifyTime = DateTime.Now;
                order.LastModifyUserId = userId;
                dbContext.Entry(order).State = EntityState.Modified;
            }

            dbContext.LogChangesDuringSave = true;
            dbContext.BusinessName = "批量导入审核不通过";
            dbContext.SaveChanges();
            return "";
        }
    }
